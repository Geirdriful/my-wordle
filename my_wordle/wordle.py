...
def main():
    ...
    while word_player.upper() != word_of_the_day and attempt < 6:
        ...
        while player_word_exists is False or len(word_of_the_day) != len(word_player):
            if player_word_exists is False:
        ...
        if not utils.find_word_of_the_day(word_player, word_of_the_day):
            ...
            ''' Affichage '''
            for letter in range(0, len(word_player)):
                if letter in return_green:
                    print("\033[0;32;40m" + word_player[letter].upper() + "\033[0m", end='')
                elif letter in return_orange:
                    print("\033[0;33;40m" + word_player[letter].upper() + "\033[0m", end='')
                else:
                    print(f'{word_player[letter].upper()}', end='')
            attempt = attempt + 1
        else:
